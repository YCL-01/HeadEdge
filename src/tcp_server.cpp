...
int receiver(int shmId) {
    ...
    for(int i=0; i<NORM_EDGE; i++){
        if((clientSock[i] = accept(sockFd, (struct sockaddr *)&clientAddr[i], (socklen_t *)&addlen))< 0) {
...
int transceiver(int shmId) {
    ...
    char risk_rate_message[MAX_BUF_SIZE] = {0, };
    char prev_risk_rate_message[MAX_BUF_SIZE] = {0, };
    ...
    while(1) {
        ...
        // printf("%s\t%s\t%s\n",current_main.scnCode,current_main.riskRate,current_main.speed);
        ...
        if(i == 0) {
            ...
            // pthread_create(&pol0, NULL, component_controller, (void *) &(current->pole0));
            // pthread_create(&pol1, NULL, component_controller, (void *) &(current->pole1));
            pthread_create(&main, NULL, ped_control, (void *) &(current->main));
            // pthread_detach(pol0);
            // pthread_detach(pol1);
            pthread_detach(main);
            ...
            // printf("pole1 current scn :%s pole1 prev scn :%s main scn : %s prev main scn: %s\n",current->pole0.scnCode,prev->pole0.scnCode, current->main.scnCode, prev->main.scnCode);
            ...
            // Risk not changed
                ...
                if(strcmp(current->main.scnCode,prev->main.scnCode)==0){
                    ...
                    if (ret != 0){
                        ...
                        // printf("this is tranceiver else, current Thread Does not exist\n");
                        pthread_create(&main, NULL, ped_control, (void *) &(current->main));
                        pthread_detach(main);
                    }
