...
def tcp_client_car(cam_id, serv_ip, serv_port):
    ...
    #연결
    print(serv_ip, serv_port)
    sock.connect((serv_ip, serv_port)) 
    
    while True:    
        s = cam_id + " " + str(random.randint(0,99))
        print(s)
        #전송
        sock.send(s.encode())
        #100ms 쉼
...
def tcp_client_ped(ped_id, serv_ip, serv_port):
    sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM) 
    #연결
    result = sock.connect((serv_ip, serv_port))
    print("result: ", result) 
    while True:
...
if __name__ == '__main__':
    ...
    p2 = Process(target=tcp_client_ped, args=('2', serv_ip, serv_port))
    p3 = Process(target=tcp_client_car, args=('3', serv_ip, serv_port))
